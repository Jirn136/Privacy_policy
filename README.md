# Privacy Policy

**Effective Date:** \[10-06-2025]

**Vibely – Private Mood Tracking**

## Overview

Vibely is a simple, distraction-free mood tracker built with your privacy in mind. We are committed to collecting as little data as possible and never use your information for anything beyond the app's core functionality.

## What We Collect

We collect only the information necessary to provide a secure and consistent user experience across your devices:

* **Email address**: Used solely for account creation and mood entry synchronization.
* **Mood entries**: Your selected mood data, stored securely to allow you to view your emotional history.

We **do not** collect your name, location, contacts, or any other personal data.

## What We Don’t Collect or Do

* We do **not** track your activity or usage.
* We do **not** use analytics tools.
* We do **not** display ads or sell any user data.
* We do **not** request unnecessary permissions.
* We do **not** collect any background or behavioral data.

## Data Storage and Security

Your data is securely stored and encrypted. We take appropriate security measures to protect your information from unauthorized access, alteration, or disclosure.

## Account and Data Management

You can request data deletion or account removal at any time by contacting us at:
**Email:** \[[janakiraman.babu@grootan.com](janakiraman.babu@grootan.com)]

Upon receiving a request, we will permanently delete your email and all associated mood entries from our servers.

## Children's Privacy

Vibely is not intended for use by children under the age of 13. We do not knowingly collect data from children. If we learn we have collected data from a child without parental consent, we will delete it.

## Changes to This Policy

If we update our privacy practices, we’ll update this policy accordingly. Any changes will be posted at this URL and, if significant, may be communicated in-app.

## Contact

For any questions or concerns about your privacy or this policy, please reach out to:
**Email:** \[[janakiraman.babu@grootan.com](janakiraman.babu@grootan.com)]
